...
function makeListTask(task, done, id) {
    //할 일이 아직 완료 상태가 아니면
    if (done == false) {
        let temp_html = `<div class='task'>${task}<i class='bi bi-trash-fill' onclick="deletetask('${id}')"></i><i class='bi bi-check-lg' onclick="changedone('${id}')"></i><i class='bi bi-pencil-fill' onclick="update_input('${id}', '${task}')"></i></div>`;
        $(".notdone").append(temp_html);
    } else { //할 일이 완료 상태면
        let temp_html = `<div class='task'>${task}<i class='bi bi-trash-fill' onclick="deletetask('${id}')"></i><i class='bi bi-check-lg' onclick="changedone('${id}')"></i><i class='bi bi-pencil-fill' onclick="update_input('${id}', '${task}')"></i></div>`;
